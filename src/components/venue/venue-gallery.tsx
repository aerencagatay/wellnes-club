...
export function VenueGallery({ images, name }: { images: string[]; name: string }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      {first && (
        <button
          className="relative col-span-2 aspect-16/9 overflow-hidden rounded-[var(--radius-media)] sm:aspect-2/1"
      ...
      {rest.map((src, index) => (
        <button
          ...
          className={`relative overflow-hidden rounded-[var(--radius-media)] ${index % 3 === 0 ? 'col-span-2 aspect-2/1 sm:col-span-1 sm:aspect-4/3' : 'aspect-4/3'}`}
