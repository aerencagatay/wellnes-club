...
export function IncludesExcludes({ includes, excludes }: { includes: string[]; excludes: string[] }) {
  ...
  return (
    <div className="grid gap-10 sm:grid-cols-2">
      <div>
        <h2 className="font-heading text-2xl text-ink">{t('includes')}</h2>
      ...
      <div>
        <h2 className="font-heading text-2xl text-ink">{t('excludes')}</h2>
        <ul className="mt-6 flex flex-col gap-3">
          {excludes.map((item) => (
            <li className="flex items-start gap-3 text-sm" key={item}>
              <Minus aria-hidden className="mt-0.5 size-4 shrink-0 text-body-light" />
              {item}
            </li>
          ))}
        </ul>
