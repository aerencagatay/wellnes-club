...
export function CampFilters() {
  ...
  return (
    <div className="flex flex-col gap-5 border-b border-sand pb-10">
      <fieldset className="flex flex-wrap items-center gap-2">
        <legend className="mr-2 type-eyebrow">{tf('program')}</legend>
        {PROGRAMS.map((value) => (
      ...
      <fieldset className="flex flex-wrap items-center gap-2">
        <legend className="mr-2 type-eyebrow">{tf('level')}</legend>
        {LEVELS.map((value) => (
